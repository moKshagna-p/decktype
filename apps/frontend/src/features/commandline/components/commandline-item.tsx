...
function ThemePreview(props: { themeName: ThemeName }) {
  ...
  return (
    ...
    >
      ...
      <div
        class="h-2.5 w-2.5 rounded-full"
        style={{ "background-color": theme().sub }}
      />
      <div
        class="h-2.5 w-2.5 rounded-full"
        style={{ "background-color": theme().text }}
      />
    </div>
  );
}
  ...
  return (
    ...
    >
      <div class="flex w-5 shrink-0 justify-center">
        <Show when={isActive()}>
          <Check
            class={cn(
              "h-4 w-4",
              props.isFocused ? "text-(--sub-alt)" : "text-(--main)",
            )}
          />
        </Show>
      </div>
      ...
      <Show when={props.scope === "themes"}>
        <ThemePreview
          themeName={props.item.id.replace("theme-", "") as ThemeName}
        />
      </Show>
    </button>
  );
